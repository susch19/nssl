...
class ShoppingListSync {
  ...
  static Future<Response> getLists(BuildContext? context) => HelperMethods.get("$listpath/batchaction/", context);
  ...
  static Future<Response> changeProductAmount(int? listId, int? productId, int? change, BuildContext? context) =>
      HelperMethods.put(
          "$listpath/$listId/products/$productId", context, ChangeProductArgs(change: change, newName: ""));

  static Future<Response> changeProductName(int? listId, int? productId, String newName, BuildContext? context) =>
      HelperMethods.put(
          "$listpath/$listId/products/$productId", context, ChangeProductArgs(change: 0, newName: newName));

  static Future<Response> changeProducts(
          int? listId, List<int?> productIds, List<int?> amount, BuildContext? context) =>
      HelperMethods.post(
          "$listpath/$listId/products/batchaction/change", context, ChangeProductsArgs(productIds, amount));
  ...
  static Future<Response> deleteContributor(int listId, int userId, BuildContext context) //Wenn lokal gespeichert
      =>
  ...
  static Future<Response> addContributor(int listId, String contributorName, BuildContext context) =>
      HelperMethods.post("$listpath/$listId/contributors/", context, AddContributorArgs(contributorName));
