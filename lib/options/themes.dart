...
class Themes with ChangeNotifier {
  ...
  static NSSLThemeData lightTheme = NSSLThemeData(
    ThemeData(
      ...
      checkboxTheme: CheckboxThemeData(
        fillColor: MaterialStateColor.resolveWith((s) {
  ...
  static Future loadTheme() async {
    late var t;
    var lastId =
        (await SharedPreferences.getInstance()).getInt("lastTheme") ?? 0;
    var userId = _ref.watch(userIdProvider);
    try {
      if (userId != null)
        t = (await DatabaseManager.database.rawQuery(
          "SELECT * FROM Themes where user_id = ?",
          [userId],
        ));
    } catch (e) {
      var temp = (await DatabaseManager.database.rawQuery(
        "SELECT * FROM Themes",
      )).first;
      ...
      var primaryBrightness =
          temp["brightness"].toString().toLowerCase().contains("dark")
          ? Brightness.dark
          : Brightness.light;
      saveTheme(
        ThemeData(
          ...
          floatingActionButtonTheme: FloatingActionButtonThemeData(
            backgroundColor: accent.shade400,
          ),
          checkboxTheme: CheckboxThemeData(
            fillColor: MaterialStateColor.resolveWith((s) {
              if (s.contains(MaterialState.selected)) {
    ...
    for (var t2 in t) {
      if (lastId == t2["id"]) {
        tm = t2["brightness"].toLowerCase().contains("dark") == Brightness.dark
            ? ThemeMode.dark
            : ThemeMode.light;
      }
      ...
      if ((t2["brightness"] as String).toLowerCase().contains("dark")) {
        ...
        var primaryBrightness =
            t2["brightness"].toString().toLowerCase().contains("dark")
            ? Brightness.dark
            : Brightness.light;
